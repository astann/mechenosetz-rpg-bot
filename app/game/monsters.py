...
MONSTERS: tuple[Monster, ...] = (
    Monster("bone_rat", "Костяная Крыса", "🐀", 0.8),
    Monster("blood_bat", "Кровавый Нетопырь", "🦇", 0.9),
    Monster("bone_guard", "Костяной Страж", "💀", 1.2),
    Monster("ghoul", "Упырь", "🧟", 1.5),
    Monster("shadow_wolf", "Сумрачный Волк", "🐺", 1.8),
    Monster("cultist", "Культист", "🕯️", 2.1),
    Monster("wraith", "Плачущий Призрак", "👻", 2.5),
    Monster("shadow_spawn", "Порождение Тени", "🗿", 3.2),
    Monster("sorcerer", "Колдун", "🔮", 3.8),
    # Акт II
    Monster("corpse_hound", "Трупная Гончая", "🦴", 2.9),
    Monster("gloom_bandit", "Сумрачный Бандит", "🗡️", 3.4),
    Monster("void_deacon", "Дьякон Пустоты", "🕍", 4.1),
    Monster("fallen_guard", "Павший Ополченец", "🔪", 3.0),
    Monster("slum_scavenger", "Нищий Падальщик", "🪓", 3.3),
    Monster("cathedral_guard", "Соборный Страж", "🛡️", 3.9),
    Monster("cathedral_warden", "Караульный Свода", "⚔️", 4.2),
    # Акт III
    Monster("ashen_revenant", "Пепельный Ревенант", "🔥", 4.8),
    Monster("black_mountain_marauder", "Мародер с Черной Горы", "📯", 5.2),
    Monster("wall_sentinel", "Дозорный Стены", "🛡️", 5.7),
    Monster("abyss_spawn", "Порождение Бездны", "🕳️", 4.6),
    Monster("black_wall_boss", "Черная Стена", "⬛", 6.2),
)
